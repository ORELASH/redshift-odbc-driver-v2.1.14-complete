...
// FWD declarations
struct _ClientSideCursorExecutor;
struct _CscStatementContext;
struct _ClientSideCursorResult;
...
struct pg_result
{
	...
	int			myntups;
	int         totalntups;
	int			mem_used;     /* Track bytes allocated for result */
	bool		capped;  /* Results mem cap reached. This isnt full result set */
	...
	struct _ClientSideCursorResult *m_cscResult; /* CSC */
    int  m_tuplesAllocatedByCscRead; /* 1 means tuples are allocated by CSC read, 0 otherwise */
};
...
struct pg_conn
{
	...
	char     *ssldefaultrootcert; /* default root certificate filename */
	...
	/* Miscellaneous stuff */
    int     rows_only; //XEN_EDITED mag count rows only, no tuples on output
    int     row_limit; //XEN_EDITED mag limit # rows displayed
    int     mem_limit; //XEN_EDITED mag maximum result size
    bool    dump2file;
	...
	PGresult   *resultForDescParam;	/* IHG: result being constructed for 't' */
	PGresult   *resultForDescRowPrep;	/* IHG: result being constructed for 'T' after Prepare get called */
	...
	int output_nbytes; //XEN_EDITED ashu maximum result size before print partial results
	bool lastseg;
	...
	// Add Proxy info
	char       *proxy_host;          /* Proxy server host. */
	char       *proxy_port;          /* Proxy server port. */
	char       *proxy_auth_type;      /* Proxy authentication type. */
	char       *proxy_credentials;   /* Proxy server credentials. */
...
extern int	pqAddTuple(PGresult *res, PGresAttValue *tup, int iStreamingCursorRows);
