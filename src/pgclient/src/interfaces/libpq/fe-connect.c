					 ...
					 int expand_dbname)
{
	...
	if(keywords != NULL && values != NULL)
	{
		...
		if(i != 0)
		{
			if(strcmp(keywords[i - 1],"libpqConnectionTracingFile") == 0)
			{
				if(values[i - 1] != NULL)
				{
					if(strcmp(values[i - 1],"stdout") == 0)
						fp = stdout;
					else
						fp = fopen(values[i - 1], "w+");
				}
				keywords[i - 1] = NULL; // Mark it as last parameter
			}
		}
	}
    ...
    // Get CSC executor per connection
    if(conn->iCscEnable)
    {
        conn->m_pCscExecutor = createCscExecutor(conn);
    }
    else
    {
        conn->m_pCscExecutor = NULL;
    }
...
fillPGconn(PGconn *conn, PQconninfoOption *connOptions)
{
	...
	tmp = conninfo_getval(connOptions, "ssldefaultrootcert");
	conn->ssldefaultrootcert = tmp ? strdup(tmp) : NULL;
...
#error SSPI is not enabled on Windows!!!
    ...
    if(tmp)
        ...
        conn->iCscEnable = FALSE;

	tmp = conninfo_getval(connOptions, "CscMaxFileSize");
    if(tmp)
	    sscanf(tmp,"%lld",&(conn->llCscMaxFileSize));
    else
        conn->llCscMaxFileSize = (4*1024);

	tmp = conninfo_getval(connOptions, "CscPath");
    if(tmp)
    {
	    strncpy(conn->szCscPath,tmp,MAX_PATH-1);
        conn->szCscPath[MAX_PATH-1] = '\0';
    }
    else
        conn->szCscPath[0] = '\0';

	tmp = conninfo_getval(connOptions, "CscThreshold");
    if(tmp)
	    sscanf(tmp,"%lld",&(conn->llCscThreshold));
    else
        conn->llCscThreshold = 1;

	tmp = conninfo_getval(connOptions, "StreamingCursorRows");
    if(tmp)
	...
	tmp = conninfo_getval(connOptions, "client_protocol_version");
	...
	conn->plugin_name = tmp ? strdup(tmp) : NULL;

	// Proxy info
	tmp = conninfo_getval(connOptions, "proxy_host");
	conn->proxy_host = tmp ? strdup(tmp) : NULL;
	tmp = conninfo_getval(connOptions, "proxy_port");
	conn->proxy_port = tmp ? strdup(tmp) : NULL;
	tmp = conninfo_getval(connOptions, "proxy_auth_type");
	conn->proxy_auth_type = tmp ? strdup(tmp) : NULL;
	tmp = conninfo_getval(connOptions, "proxy_credentials");
	conn->proxy_credentials = tmp ? strdup(tmp) : NULL;

	// Redshift Native Auth
	tmp = conninfo_getval(connOptions, "idp_type");
	conn->idp_type = tmp ? strdup(tmp) : NULL;
	tmp = conninfo_getval(connOptions, "provider_name");
	conn->provider_name = tmp ? strdup(tmp) : NULL;
	tmp = conninfo_getval(connOptions, "web_identity_token");
	conn->idp_token = tmp ? strdup(tmp) : NULL;

	tmp = conninfo_getval(connOptions, "token_type");
...
#endif   /* HAVE_UNIX_SOCKETS */
	{
		...
		else if (addr->ss_family == AF_INET)
		{
			if (inet_net_ntop(AF_INET,
				...
				strncpy(host_addr, "???",sizeof(host_addr));
		}
#ifdef HAVE_IPV6
		...
		case CONNECTION_NEEDED:
			{
				...
				while (conn->addr_cur != NULL)
				{
					 ...
					 */
					if (!conn->proxy_host || !*conn->proxy_host)
						rc = connect(conn->sock, addr_cur->ai_addr,
										addr_cur->ai_addrlen);
					else
						rc = connect_using_proxy(conn->sock, conn);
		...
		case CONNECTION_AWAITING_RESPONSE:
			{
				 ...
				 * - NegotiateProtocolVersion ('v') in cases when server does
				 * not support protocol compression. This message is emitted
				 * by padb versions that do not understand compression
				 * negotiation. They send 'v' to indicate that not all
				 * connection parameters were understood, which is fine for us.
				...
				if (beresp == 'z')	/* Switch on compression */
				{
					zpq_compressor *compressors;
					size_t		n_compressors;
					char	   *resp = (char*)malloc(msgLength);
					 ...
					 * algorithms. Proceed without compression.
					 * TODO(xformmm): Add some indication to user here.
					 */
				...
				else if (conn->n_compressors > 0 && !conn->zpqStream)
				{
					 ...
					 * So far we silently ignore this.
					 * TODO(xformmm): Add some indication to user here.
					 */
				}

				if (beresp == 'v')	/* NegotiateProtocolVersion */
				{
					uint32 protocol_version = 0;
					pqGetInt((int *) &protocol_version, 4, conn);
					if (protocol_version > PG_PROTOCOL_LATEST)
					{
						appendPQExpBuffer(&conn->errorMessage,
										  libpq_gettext("server returned unrecognized protocol version: %d\n"),
										  protocol_version);
						goto error_return;
					}
					conn->inCursor += msgLength - 4; /* other data is ignored */
					conn->inStart = conn->inCursor;
					goto keep_going;
				}
...
freePGconn(PGconn *conn)
{
	...
	if (conn->plugin_name)
		free(conn->plugin_name);

	// Proxy info
	if (conn->proxy_host)
		free(conn->proxy_host);
	if (conn->proxy_port)
		free(conn->proxy_port);
	if (conn->proxy_auth_type)
		free(conn->proxy_auth_type);
	if (conn->proxy_credentials)
		free(conn->proxy_credentials);
	...
	if (conn->idp_type)
		free(conn->idp_type);
	if (conn->provider_name)
		free(conn->provider_name);
	if (conn->idp_token)
		free(conn->idp_token);
    ...
    // Free csc executor
    if(conn->m_pCscExecutor)
        conn->m_pCscExecutor = releaseCscExecutor(conn->m_pCscExecutor);

    // Copy from closePGconn
...
/*
 * pqCloseConnection: properly close a connection to the backend. Added for ODBC -- IHG
 * 
 */
void
pqCloseConnection(PGconn *conn)
{
	if (conn)
	{
		closePGconn(conn);
...
/*
 * pqFreeConnection: frees the PGconn data structure so it shouldn't be re-used after this. Added for ODBC -- IHG
 * 
 */
void
pqFreeConnection(PGconn *conn)
{
	if (conn)
	{
		freePGconn(conn);
 ...
 * just as dangerous.  We avoid snprintf here for that reason.  Building up
 * error messages with strncpy/strncat is tedious but should be quite safe.
...
/*
* 
* pqDigestResponsePacketSend()
*
* pack_type: the single-byte message type code.
*
* digest, digest_len: the digest and its length.
*
* nonce, nonce_len: the client nonce and its length.
*
* RETURNS: STATUS_ERROR if the write fails, STATUS_OK otherwise.
* SIDE_EFFECTS: may block.
*
...
*/
int
pqDigestResponsePacketSend(PGconn *conn, char pack_type,
...
//	int total_len;
	...
	/* Send message length */
/*	total_len = 4 + 4 + digest_len + 4 + nonce_len;
	if (pqPutInt(total_len, 4, conn))
		return STATUS_ERROR;
*/

	/* Send the message body. */
	if (pqPutInt(digest_len, 4, conn))
		return STATUS_ERROR;
	if (pqPutnchar(digest, digest_len, conn))
		return STATUS_ERROR;
	if (pqPutInt(nonce_len, 4, conn))
		return STATUS_ERROR;
	if (pqPutnchar(nonce, nonce_len, conn))
		return STATUS_ERROR;
...
void
PQsetRowsOnly(PGconn *conn, int limit)
{
	if (conn == NULL)
		return;
	conn->rows_only = limit;
}

void
PQsetRowLimit(PGconn *conn, int limit)
{
	if (conn == NULL)
		return;
	conn->row_limit = limit;
}

void
PQsetMemLimit(PGconn *conn, int mem_limit)
{
	if (conn == NULL)
		return;
	conn->mem_limit = mem_limit;
}

void
PQsetDump2File(PGconn *conn, int state)
{
	if (conn == NULL)
		return;
	conn->dump2file = state;
}

void
PQsetOutputNbytes(PGconn *conn, int nbytes)
{
        if (conn == NULL)
                return;
        conn->output_nbytes = nbytes;
}
...
pqGetHomeDirectory(char *buf, int bufsize)
{
	...
	if (SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, tmppath) != S_OK)
		return false;
	snprintf(buf, bufsize, "%s/redshift", tmppath);
	return true;
#endif
}
...
/*
 * Get the "C" part of the error message. Added for ODBC -- IHG
 * 
 */
char *pqGetNativeSqlState(PGconn *conn)
{
	return (conn) ? conn->last_sqlstate : NULL;
}
