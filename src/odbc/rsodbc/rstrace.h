...
// This Macros is a helper, used by member methods in RsTrace
#define WRITE_INTO_FILE(NEWLINE)                                               \
    ...
    va_end(args_copy);                                                         \
    buffer.resize(prev + size, ' ');                                           \
    vsnprintf(&buffer[prev], size, fmt, args);                                 \
    buffer.pop_back();                                                         \
    if (NEWLINE) {                                                             \
        buffer.push_back('\n');                                                \
    }                                                                          \
...
#define WRITE_INTO_FILE_WITH_ARG_LIST(NEWLINE, args)                           \
    int prev = buffer.empty() ? 0 : buffer.size() - 1;                         \
    va_list args_copy;                                                         \
    va_copy(args_copy, args);                                                  \
    const auto size = std::vsnprintf(nullptr, 0, fmt, args_copy) + 1;          \
    va_end(args_copy);                                                         \
...
/*
This class implements ODBC API call logging.
Throwout its lifetime, the object of this class
buffers all the necessary data sent to it.
The actual logging of the buffer is carried out during object destruction.
Use the class for small jobs, like API call start and end. This way, the logger
will have enough log likes with the help of which, it is possible to, for example,
measure (using logger's timestamp usually available in its log format)
lifetime of an API call consistently.
*/

class RsTrace {
  private:
    std::string buffer;
    const char *TAG = "ODBCAPI";

    int getLogLevel() {
        // This logic should change in future if we support changing log level
        // inflight
        static int logLevel = getRsLoglevel();
        return logLevel;
    }

    /*
        Do not log if logger is OFF, or Logger is set to log only lower
        level lines.
    */
    virtual int logWhat(int iCallOrRet) {
        /*
            Note: condition "getRsLoglevel() < m_level" technically
            covers "getRsLoglevel() == LOG_LEVEL_OFF" too but we keep it for
            visibility.
        */
...
class RsTraceNoop : public RsTrace {
public:
